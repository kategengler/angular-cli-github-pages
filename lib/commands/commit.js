            ...
            inquire.prompt([{
              ...
              req.on('response', function(response) {
                if (response.statusCode === 201) {
                  resolve(runCommand(`git remote add origin git@github.com:${answers.ghUserName}/${projectName}.git`).
                  then(() => runCommand("git checkout --orphan gh-pages && git rm --cached -r . && git add .gitignore && git clean -f -d && git commit -m 'initial gh-pages commit' && git checkout master")))
                } else {
